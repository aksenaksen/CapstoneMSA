package com.example.capstone.user.constant;

public class CustomStatusCode {
    public static final int NOT_ALLOWED_USERNAME_FORM = 601;
    public static final int NOT_ALLOWED_PASSWORD_FORM = 602;
    public static final int NOT_ALLOWED_EMAIL_FORM = 603;
    public static final int NOT_ALLOWED_KEY = 604;
    public static final int ALREADY_EXIST_USERNAME = 620;
    public static final int ALREADY_EXIST_PHONE_NUM = 621;

    public static final int NOT_ALLOWED_NICKNAME_FORM = 624;
    public static final int NOT_ALLOWED_PHONENUMBER_FORM = 625;


    public static final int NOT_ALLOWED_CODE_FORM = 605;

    public static final int NOT_MATCHED_CODE_VALUE = 606;
    public static final int BAD_GATEWAY_IN_IDENTIFICATION= 607;

    public static final int BAD_REQUEST_SESSIONID = 608;


}
